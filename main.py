...
def calculate_commits_by_path(paths: list[str], days: int) -> list[tuple[str, int]]:
    res = {}
    for path in paths:
...
def print_results(commits_by_path: list[tuple[str, int]]):
    print("----------------")
...
def get_commits_for_repo(repo_path="./", days=30) -> list[tuple[date, int]]:
    repo = Repo(repo_path)
...
def get_total_commits(commit_counts: list[tuple[date, int]]) -> int:
